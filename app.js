...
//Top Search
let topSearchField = document.querySelector('#searchField');
let appUsers = ['Victoria Chambers', 'Dale Byrd', 'Dawn Wood', 'Dan Oliver'];

document.querySelector("#searchButton").addEventListener('click', function() {
	if (topSearchField.value === "") {	
		alert('Please type in a name to search.');
	} else if (appUsers.indexOf(topSearchField.value) >= 0) {
		alert('User: ' + topSearchField.value + ' exists!');
	} else {
		alert('User: ' + topSearchField.value + ' doesn\'t exist.');
	};
...
// Messaging Section //
	...
	document.querySelector("#searchField").addEventListener("click", function() {
		document.querySelector("#searchFieldSuggestions").style.display = "block";
	});

	document.querySelector("#userField").addEventListener("click", function() {
		document.querySelector("#userFieldSuggestions").style.display = "block";
	});

	function hideAutocompleteResults() {
		document.querySelector("#searchFieldSuggestions").style.display = "none";
		document.querySelector("#userFieldSuggestions").style.display = "none";
	}
	...
	document.querySelector("#searchField").addEventListener("autoComplete", event => {
		console.log(event);
	});
	...
	function createAutoComplete(searchBox, idName) {
		new autoComplete({
			...
			selector: searchBox,                 // Input field selector              | (Optional)
			...
			resultsList: {                       // Rendered results list object      | (Optional)
				...
				destination: document.querySelector(searchBox),
				position: "afterend",
				element: "ul"
			},
	...
	createAutoComplete("#searchField", "searchFieldSuggestions");
	createAutoComplete("#userField", "userFieldSuggestions");
